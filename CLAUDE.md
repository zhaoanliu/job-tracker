...
**All Vercel deploys are gated on CI passing.** `vercel.json` sets `ignoreCommand: exit 1` to disable Vercel's auto-deploy entirely. `lint.yml` triggers `vercel deploy --prod` as its final step, only on success + push to main. No preview deploys — not needed for a single-reviewer project. `VERCEL_ORG_ID` and `VERCEL_PROJECT_ID` are hardcoded from `.vercel/project.json` — no additional secrets needed for those.
...
- **`actions/github-script` must use `github-token: ${{ secrets.GH_PAT }}`** when calling `createDispatchEvent` to trigger `repository_dispatch` on the same repo. The default `GITHUB_TOKEN` has `contents: read` scope and the API call returns 403 silently — the dispatch is never sent and the target workflow never runs. All four "Trigger CI auto-fix on failure" steps in `lint.yml`, `e2e.yml`, `e2e-local.yml`, and `migrate.yml` must specify `github-token: ${{ secrets.GH_PAT }}`.
