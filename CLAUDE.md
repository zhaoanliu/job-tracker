...
- **Concurrency groups in CI workflows called via `workflow_call` must use `github.workflow_ref` as the group key** — `github.run_id` appears to be the **caller's** run ID (same for all 4 workflow_call invocations from cd.yml), so using it as the group key causes all 4 called workflows to compete for one slot — exactly 2 are cancelled per CD run. `github.workflow_ref` (e.g. `owner/repo/.github/workflows/lint.yml@refs/heads/main`) is unique per workflow file, so each called workflow gets its own isolated slot. For PR events the ref is `refs/pull/N/merge`, which scopes the group to the specific PR — `cancel-in-progress: true` then correctly cancels stale runs on new commits. Use: `group: ${{ github.workflow_ref }}`, `cancel-in-progress: ${{ github.event_name == 'pull_request' }}`.
