...
- **Concurrency groups in CI workflows must not use a shared key for `workflow_call` events** — `group: ${{ github.workflow }}-${{ github.ref }}` works for PR events but causes `lint` and `migrate-validate` to be cancelled when `cd.yml` calls all 4 CI workflows in parallel via `workflow_call` (all share `refs/heads/main`, causing concurrency collisions). Use `github.run_id` as the group for non-PR events: `group: ${{ github.event_name == 'pull_request' && format('{0}-{1}', github.workflow, github.ref) || github.run_id }}`. This gives each `workflow_call` invocation its own isolated slot while preserving cancel-in-progress for PR stale-run cleanup.
