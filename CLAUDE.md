...
- **GitHub search takes ~15s to index a newly-created issue.** If the `repository_dispatch` run searches by Sentry ID immediately after `sentry[bot]` creates the issue, it won't find it and creates a duplicate. The workflow adds `sleep 15` before searching in the `repository_dispatch` path.
