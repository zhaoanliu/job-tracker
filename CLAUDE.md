...
**Doc-only changes and required checks.** `cd.yml`, `lint.yml`, `test.yml`, `e2e.yml`, and `migrate-validate.yml` all have `paths-ignore` for `**/*.md` and `docs/**` on their `pull_request` triggers. **`paths-ignore` does NOT satisfy required status checks** — when the filter skips the workflow, no check run is created at all, so required checks stay in a permanent `pending` state and the PR is blocked from merging. GitHub only treats a check as "skipped" (which satisfies required checks) when a *job* is skipped via a job-level `if:` condition, because that path still creates a "skipped" check run. The current setup relies on `enforce_admins: false` so repo owners can bypass the block on doc-only PRs; if doc-only PRs need to merge without admin override, replace `paths-ignore` with a job-level `if: !contains(...)` guard, or just let CI run on doc PRs (it passes quickly). `workflow_call` triggers have no path filter — when `cd.yml` triggers for a code push it still calls all 4 workflows unconditionally.
