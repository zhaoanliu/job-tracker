...
- **`GITHUB_TOKEN` also suppresses `pull_request: [synchronize]` triggers on direct branch pushes** — when a bot workflow pushes a fix commit directly to an existing PR branch (`git push origin HEAD:<branch>`), the push is attributed to `github-actions[bot]` and GitHub silently suppresses the `pull_request` CI trigger. The PR's check statuses reset to "Waiting for status to be reported" but no new CI run starts — the PR is stuck. Fix: call `git remote set-url origin "https://x-access-token:${GH_PAT}@github.com/${GITHUB_REPOSITORY}.git"` immediately before the push so it is attributed to the human actor. This is distinct from the `gh pr create` case — `GH_TOKEN="${GH_PAT}" gh pr create` handles new PRs; the `git remote set-url` pattern is needed for direct pushes to existing PR branches.
...
2. **Create a git worktree** before making any edits — run `git worktree add ../job-tracker-N -b <branch-name> origin/main` and do all subsequent work (file edits, commits, pushes) from that directory. Use `origin/main` (not local `main`) so the worktree is always based on the latest remote state even if local main is behind. **Never edit files in the main working directory, even temporarily** — do not edit-then-copy into the worktree; only edit inside the worktree. This keeps parallel sessions fully isolated on disk so uncommitted changes from one session can't bleed into another. Branch naming: `fix/issue-N-<timestamp>`, `feat/issue-N-<slug>`, or `docs/issue-N-<slug>` (branch prefixes follow conventional commits; issue title prefixes do not)
