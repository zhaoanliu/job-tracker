...
| Generic (e.g. Expedia) | Any career page that isn't matched by the above | `schema.org/JobPosting` JSON-LD embedded in page HTML — generic fallback handler runs after all ATS-specific checks | `expedia-job.json` (Expedia Group, R-105467-3) |
...
- **Generic handler (Expedia / Phenom / WordPress-based career sites)**: `extractGenericJobPostingFromPage` is the final JSON-LD fallback, after Workday and Uber checks. It extracts `title`, `hiringOrganization.name`, `identifier` (string form or `{value}` object form), `jobLocation.address` (single or array), `datePosted`, and `employmentType`. ATS-specific handlers take priority — the generic handler only fires for pages that don't match Workday/Uber URL patterns. Expedia's `identifier` is a plain string ("R-105467-3"), unlike Workday which uses `{value: "..."}`.
