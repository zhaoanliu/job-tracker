...
| Google Careers | `www.google.com/about/careers/applications/jobs/results/{id}-{slug}` and `careers.google.com/jobs/results/{id}-{slug}` | `AF_initDataCallback({key: 'ds:0', ..., data:[...]})` block embedded in page HTML — no JSON-LD, no public API | `google-careers-job.json` (Google, job 92918703267422918) |
...
- **Google Careers**: JS-rendered SPA with no JSON-LD and no public JSON API. Google injects the full job data server-side via `AF_initDataCallback({key: 'ds:0', hash: '...', data:[[...]], sideChannel: {}})`. The `data[0]` array contains: `[0]` job ID, `[1]` title, `[2]` sign-in URL, `[3]` responsibilities `[null, html]`, `[4]` qualifications `[null, html]`, `[7]` company, `[9]` locations `[[displayName, ...], ...]`, `[10]` about-the-job `[null, html]`. The fixture is the `data` outer array (not the full callback string); the test wraps it with `googleCareersPage(data)` which generates the callback script tag. `careers.google.com` redirects to `www.google.com/about/careers/...` but returns identical HTML, so both hostname patterns must be detected for the flag to be set before the fetch. Content fields are already decoded HTML (no entity decoding needed). The `data[0]` array has 21 fields in the real fixture; only indices 1, 3, 4, 7, 9, 10 are used.
